...
const StudentForm = ({
  ...
  useEffect(() => {
    if (state.success) {
      toast.success(
        state.message || `Student has been ${type === "create" ? "created" : "updated"}!`
      );
      setOpen(false);
      router.refresh();
    } else if (state.error) {
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={onSubmit}>
      ...
      <button type="submit" className="bg-hsYellow text-black font-semibold hover:bg-yellow-400 p-2 rounded-md transition-colors">
